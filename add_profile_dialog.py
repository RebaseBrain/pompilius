...
from pompilius import Provider, get_available_providers, set_margins, MAX_TIMEOUT_MS, bus, get_existing_profiles
import json

class AddProfileDialog(Gtk.Window):
    def __init__(self, parent_extension, transient_for=None, refresh_callback=None):
        super().__init__(title="Добавление хранилища", modal=True, default_width=500, default_height=700, transient_for=transient_for)
    ...
    def setup_ui(self):
        ...
        self.entry_profile_name = Gtk.Entry(placeholder_text="Например, Мой Яндекс.Диск")
    ...
    def on_create_clicked(self, button):
        ...
        if not profile_title:
            self.parent_ext.show_error_dialog(self, "Ошибка", "Введите название профиля")
            return
            
        profiles = get_existing_profiles()
        if profile_title in profiles:
            self.parent_ext.show_error_dialog(self, "Ошибка", f"Профиль '{profile_title}' уже существует")
            return

        self.btn_create.set_sensitive(False)
        self.btn_create.set_label("Создание...")

        result_params = {key: entry.get_text() for key, entry in self.entries_map.items()}
        try:
            ...
            bus.call(
                ...
                'org.zbus.pompiliusd',
                'CreateProfile',
                GLib.Variant('(sss)', (profile_title, self.selected_provider_name, params_json_string)),
                None, Gio.DBusCallFlags.NONE, MAX_TIMEOUT_MS, None,
